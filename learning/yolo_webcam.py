...
try:
    while True:
        ...
        for box in results[0].boxes:
            ...
            # Print detection
            print(f"{class_name} | {confidence:.2f} | Threat = {is_threat}")
            ...
            # ----------------------------------------
            # Automatic Database Logging (Inside Detection Loop)
            # ----------------------------------------
            if is_threat:
                log_time = time.time()

                if (
                    class_name not in last_logged
                    or log_time - last_logged[class_name] >= LOG_COOLDOWN
                ):
        ...
        # Manual Screenshot
            ...
            timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
            filename = f"snapshots/manual_{timestamp}.jpg"
